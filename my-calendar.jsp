...
.__web-inspector-hide-shortcut__, .__web-inspector-hide-shortcut__ *, .__web-inspector-hidebefore-shortcut__::before, .__web-inspector-hideafter-shortcut__::after
	...
	<style>
		...
		.fc-daygrid-day-number {
		    font-size: 1.2em;  /* 날짜 숫자 크기 증가 */
		}

		.fc-daygrid-day-events {
		    min-height: 2em;  /* 이벤트 영역 최소 높이 설정 */
		}
		...
		.fc-header-toolbar {
		    ...
		    flex-wrap: wrap;
		    margin-bottom: 20px;
		    font-size: 1.1em;  /* 툴바 글꼴 크기 증가 */
		}

		.fc-toolbar-title {
		    font-size: 1.8em;  /* 타이틀 크기 증가 */
		    flex-grow: 1;
		    text-align: center;
		}
...
<!-- FullCalendar CSS 파일 포함 -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/fullcalendar@6.1.15/main.min.css">
<!-- 날짜 클릭 시, 해당 날짜 이벤트 보여주는 Js-->
<script>
        document.addEventListener('DOMContentLoaded', function() {
            var calendarEl = document.getElementById('calendar');
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function(info) {
                    displayMemo(info.dateStr); // 날짜 클릭 시 메모 표시 함수 호출
                }
            });
            calendar.render();
            ...
            function displayMemo(date) {
                ...
                var memoText = getMemoForDate(date); // 해당 날짜의 메모를 가져오는 함수 호출
                memoContainer.innerHTML = `<h3>${date}</h3><p>${memoText}</p>`;
            }

            function getMemoForDate(date) {
                // 여기에서 실제로 해당 날짜에 저장된 메모를 가져오는 로직을 구현해야 합니다.
                // 예를 들어, 서버로부터 AJAX 요청을 보내거나 로컬 스토리지에서 가져오는 방식 등을 사용할 수 있습니다.
                // 이 예제에서는 간단히 하기 위해 더미 데이터를 반환합니다.
                // 실제로 사용할 때는 데이터베이스 등을 연동하여 해당 날짜의 메모를 가져오는 방식으로 구현해야 합니다.
                var dummyData = {
                    '2024-07-15': '오늘의 일정: 회의 진행'
                    // 여기에 다른 날짜의 메모도 추가할 수 있습니다.
                };
                return dummyData[date] || '메모가 없습니다.';
            }
        });
    </script>
<!-- 달력 형태 Js-->
