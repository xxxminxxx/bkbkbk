...
<div class="py-5 bg-light">
    <div class="container bshelf py-4">
        <h1 class="text-center mb-4">📚 나의 서재 📚</h1>
		<div style="display: flex; justify-content: center; margin-bottom: 4rem;">
		  <h6>읽은 책을 확인하고, 읽고 있는 책의 진행 상황을 기록하며 메모도 남길 수 있어요.</h6>
		</div>
        <c:choose>
                                        ...
                                        <div id="character-stages" class="d-flex justify-content-between"> 
                                            ...
                                            <div class="character-section col-md-6 d-flex justify-content-center align-items-center" style="width: 45%;">
                                                <div class="stage-character p-3 text-center user-level-info">
                                                  ...
                                                  <!--  <p>다음 단계까지 남은 책: <span id="booksToNextLevel">${booksToNextLevel}</span>권</p>-->
      ...
      <div class="modal-dialog">
         <div class="modal-content">
            ...
            <div class="modal-footer">
               <button type="button" class="btn btn-secondary" data-dismiss="modal" style="width: 97px; height: 50px; margin-right: 10px;">닫기</button>
               ...
               <button type="button" class="btn btn-primary" id="saveMemoButton" style=" height: 50px; margin-left: 10px;">저장</button>
            </div>
         </div>
      </div>
   </div>
         ...
         <%@ include file="../footer.jsp" %>
		   ...
		   <script>
		    ...
		    console.log("전역 변수 확인: ", readBooks,readingBooks, wishBooks);
		     ...
		     $(document).ready(function() {
		       ...
		       // 모든 툴팁 초기화
		         ...
		         let shelfNum;  // #[수정] 외부에서 사용할 수 있도록 변수를 전역으로 설정
		       ...
		       $(document).on('click', '.delete-book', function() {
		           ...
		           var $bookItem = $(this).closest('.book-item'); // 삭제할 책 항목
		                   ...
		                   success: function(result) {
		                           if(result === 'success') {
		                               $bookItem.fadeOut(300, function() {
		                                   ...
		                                   updateBookArrays(); // 책 목록 배열 갱신
		                               });
		                               Swal.fire('삭제 완료', '책이 삭제되었습니다.', 'success');
		                           } else {
		                               Swal.fire('삭제 실패', '책 삭제에 실패했습니다.', 'error');
		                           }
		                       },
		                       error: function() {
		           ...
		           var noBookHtml = `
		               <div class="without-data">
		                   <div class="d-flex justify-content-center align-items-center" style="height: 100px;">
		                       <h4 class="no-books-message text-center m-0">책이 없습니다!</h4>
		                   </div>
		                   <div class="d-flex justify-content-center">
		                       <a href="/" class="btn btn-primary mx-2">베스트셀러 구경하러 가기</a>
		                       <a href="/pages/book-categorySearch" class="btn btn-secondary mx-2">카테고리별 책 구경하기</a>
		       ...
		       function refreshCurrentTab() {
		           ...
		           $.ajax({
		               ...
		               success: function(response) {
		                   // 새로운 내용으로 탭을 업데이트하되, 상단 영역은 제외
		                   var $newContent = $(response);
		                   $newContent.children().first().remove(); // 새 컨텐츠의 상단 영역 제거
		         ...
		         // #[수정] 읽고 있는 책 수정 버튼 클릭 이벤트
		         $(document).on('click', '.edit-progress', function() { // #[수정] 이벤트 위임 방식으로 변경
		      ...
		      $(document).on('click', '#saveProgress', function(e) {
		           ...
		           // ##[수정] 책을 완독한 경우에만 별점 입력 모달 표시 및 데이터에 추가
		           if (isCompleted) {
		               Swal.fire({
		                   title: '책을 다 읽으셨군요!',
		                   text: '이 책에 대한 별점을 입력해주세요.',
		                   input: 'range',
		                   inputAttributes: {
		                       min: 1,
		                       max: 5,
		     ...
		     function saveBookProgress(data) {
		         $.ajax({
		             ...
		             success: function(response) {
		                 if(response.status === "success") {
		                     ...
		                     refreshAllTabs(); // 모든 탭 새로고침 (필요한 경우)
		                 } else {
		                     Swal.fire({
		                         icon: 'error',
		                         title: '저장 실패',
		       ...
		       function refreshAllTabs() {
		           refreshTab('nav-read');  // 읽은 책 탭
		           refreshTab('nav-reading');  // 읽고 있는 책 탭
		           refreshTab('nav-wishlist');  // 읽고 싶은 책 탭
		       }
			 ...
			 function refreshTab(tabId) {     
			     ...
			     $.ajax({
			         ...
			         data: { tabId: tabId },
			         success: function(response) {
			             // 정렬 버튼을 포함한 상단 영역 전체를 복제
			             let $topSection = $tab.children().first().clone();
			             
			             // 새로운 내용으로 탭 업데이트
			             $tab.html(response);
			             
			             // 복제한 상단 영역(정렬 버튼 포함)을 탭의 맨 앞에 다시 추가
			             $tab.prepend($topSection);
			         ...
			         error: function(xhr, status, error) {
			             console.error(`${tabId} 탭 새로고침 실패:`, error);
			         }
			     });
			 }
		    ...
		    function sortEntries(order) {
		           ...
		           updateBookArrays(); // 정렬 전 배열 업데이트

		           // 현재 활성화된 탭에 따라 적절한 책 목록과 컨테이너 선택
