			...
			<div class="container py-4 ">
				<div class="py-5">
					<div class="container mt-5 text-center" style="max-width: 900px;">
						...
						<!-- 도서관 검색 폼 -->
						<form  method="get" class="mb-4" id="submitBook" action="/findLibraries">
						...
						<c:if test="${empty libraries}">
							<c:if test="${empty doName}">
								<p class="alert alert-info">"<u>${bookTitle}</u>"을 찾을 시,도를 선택하세요.</p>
							</c:if>
							<c:if test="${not empty doName}">
							...
							<c:when test="${not empty libraries}">
								...
								<div class="table-responsive" style="height: 25em;">
									<table class="table table-striped">
										<thead>
										<tr>
											<th class="col-xl-2">도서관 이름</th>
											<th class="col-xl-2">주소</th>
											<th class="col-xl-2">전화번호</th>
											<th class="col-xl-2">웹사이트</th>
											<th class="col-xl-2">도서상태</th>
											<th class="col-xl-2">위치찾기</th>
										</tr>
										</thead>
										<tbody>
										<!-- 각 도서관 정보를 행으로 표시 -->
										<c:forEach items="${libraries}" var="library">
											<tr>
												...
												<td><a href="${library.url}" class="text-purple" target="_blank">바로가기</a></td>
												<c:choose>
													<c:when test="${library.loanAvailable eq 'Y'}">
														<td>대여 가능</td>
													</c:when>
													<c:otherwise>
														<td>대여 불가</td>
